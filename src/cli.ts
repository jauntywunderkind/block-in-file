...
export default async function main(args = Deno.args) {
  return await new Command()
    ...
    .option("-i, --input <input:file>", "Input file to read contents from, or - from stdout", {
      default: "-",
    })
    .option(
    ...
    .option(
      "-b, --before [before:string]",
      "String or regex to insert before, or at beginning if no argument",
    )
    .option(
      "-a, --after [after:string]",
      "String or regex to insert after, or at end if no argument",
    )
